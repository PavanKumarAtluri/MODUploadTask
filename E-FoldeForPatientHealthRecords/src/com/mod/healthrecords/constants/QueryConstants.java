...
public class QueryConstants {
	...
	public static final String INSERT_PATIENT_HEALTH_RECORD_DETAILS_QUERY="insert into phr_tab(PHR_ID,PATIENT_ID,DOCTOR_ID,PHR_UPLOADED_DATE,PHR_TYPE,PHR_UPLOADED_PATH_ORIGINAL,PHR_UPLOADED_PATH_PDF,PHR_DESCRIPTION) values(?,?,?,?,?,?,?,?)";
	...
	public static final String GET_PHR_DETAILS_BY_PATIENT_ID_AND_PHR_TYPE="select p.PHR_ID,PATIENT_ID,p.DOCTOR_ID,to_char(TO_TIMESTAMP(p.PHR_UPLOADED_DATE),'YYYY-MM-DD HH:MI:SS'), p.PHR_TYPE,p.PHR_UPLOADED_PATH_ORIGINAL,p.PHR_UPLOADED_PATH_PDF,p.PHR_DESCRIPTION,d.DOCTOR_NAME,d.DOCTOR_SPECIALIZATION,p.PATIENT_PRESCRIPTION from phr_tab p, doctors_tab d where p.doctor_id=d.doctor_id and p.patient_id=? and p.phr_type=? order by p.PHR_ID desc";
	public static final String GET_DOCTOR_REPORT_BY_DOCTOR_ID="select ph.PHR_ID,ph.doctor_id, p.patient_id, p.patient_name, p.patient_age, p.patient_sex, to_char(TO_TIMESTAMP(ph.PHR_UPLOADED_DATE),'YYYY-MM-DD HH:MI:SS'), ph.phr_type, ph.phr_uploaded_path_original, ph.phr_uploaded_path_pdf, ph.phr_description from phr_tab ph inner join patient_tab p on ph.patient_id = p.patient_id where ph.doctor_id=? order by ph.PHR_ID desc";
	public static final String GET_DOCTORS_DETAILS="select DOCTOR_ID,DOCTOR_NAME,DOCTOR_SPECIALIZATION from doctors_tab";
	public static final String GET_ALL_REPORTS_BY_PATIENT_ID="select p.PHR_ID,PATIENT_ID,p.DOCTOR_ID,to_char(TO_TIMESTAMP(p.PHR_UPLOADED_DATE),'YYYY-MM-DD HH:MI:SS'), p.PHR_TYPE,p.PHR_UPLOADED_PATH_ORIGINAL,p.PHR_UPLOADED_PATH_PDF,p.PHR_DESCRIPTION,d.DOCTOR_NAME,d.DOCTOR_SPECIALIZATION,p.PATIENT_PRESCRIPTION from phr_tab p, doctors_tab d where p.doctor_id=d.doctor_id and p.patient_id=? order by p.PHR_ID desc";
